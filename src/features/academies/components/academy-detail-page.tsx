...
import {
  useAcademy,
  useAddAthleteToAcademy,
  useRemoveAthleteFromAcademy,
  useUpdateAcademy,
...
export function AcademyDetailPage({ academyId }: AcademyDetailPageProps) {
  ...
  const academyQuery = useAcademy(activeCompetitionId, academyId);
  const academy = academyQuery.data;
  const academyAthletes = useMemo(() => academy?.athletes ?? [], [academy?.athletes]);
  const athleteSearchQuery = useAthletes(
    debouncedSearch.trim().length >= 2 ? activeCompetitionId : null,
    debouncedSearch.trim(),
  );
  const addAthleteMutation = useAddAthleteToAcademy(activeCompetitionId, academyId);
  const removeAthleteMutation = useRemoveAthleteFromAcademy(
    activeCompetitionId,
    academyId,
  );
  const updateAcademyMutation = useUpdateAcademy(activeCompetitionId, academyId);

  const suggestions = useMemo(() => {
    const athletes = athleteSearchQuery.data ?? [];
    const currentIds = new Set(academyAthletes.map((athlete) => athlete.id));

    return athletes.filter((athlete) => !currentIds.has(athlete.id));
  }, [athleteSearchQuery.data, academyAthletes]);
  ...
  async function handleAddAthlete(athlete: Athlete) {
    try {
      ...
      toast({
        ...
        description: `${athlete.name} agora faz parte de ${academy?.name ?? 'a academia'}.`,
        variant: 'success',
      });
    } catch (error) {
      toast({
        title: 'Falha ao vincular atleta',
        description:
  ...
  async function handleConfirmRemove() {
    ...
    try {
      ...
      toast({
        ...
        description: `${athleteToRemove.name} foi removido da academia.`,
        variant: 'success',
      });
      setAthleteToRemove(null);
    } catch (error) {
      toast({
        title: 'Falha ao desvincular atleta',
        description:
          error instanceof Error ? error.message : 'Tente novamente.',
        variant: 'destructive',
      });
  ...
  async function handleUpdateAcademy(values: AcademyFormValues) {
    try {
      await updateAcademyMutation.mutateAsync(values);
      ...
      toast({
        title: 'Falha ao atualizar academia',
        description:
          error instanceof Error ? error.message : 'Tente novamente.',
        variant: 'destructive',
      });
  ...
  return (
    <div className="space-y-6">
      ...
      {hasHydrated && !activeCompetitionId && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="p-6 text-amber-900">
            Selecione uma competição no switcher superior para visualizar a academia.
          </CardContent>
        </Card>
      )}

      {activeCompetitionId && academyQuery.isLoading && (
        <Card>
          <CardContent className="p-6 text-slate-600">
            Carregando academia...
          </CardContent>
      ...
      {activeCompetitionId && academyQuery.isError && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="p-6 text-red-700">
            {academyQuery.error instanceof Error
              ? academyQuery.error.message
              : 'Falha ao carregar academia.'}
          </CardContent>
        </Card>
      )}

      {activeCompetitionId && !academyQuery.isLoading && !academyQuery.isError && !academy && (
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="p-6 text-amber-900">
            Academia não encontrada na competição ativa.
          </CardContent>
        </Card>
      )}
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-slate-600">
                  Adicione atletas pela busca, acompanhe o status de pesagem e gerencie a filiação de cada membro.
                </p>
              </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                <div className="inline-flex items-center gap-3 rounded-2xl bg-slate-100 px-4 py-3 text-sm font-semibold text-slate-700">
                  ...
                  <span>
                    {academyAthletes.length} {academyAthletes.length === 1 ? 'atleta' : 'atletas'}
                  </span>
                </div>
            ...
            <CardContent className="space-y-4 p-6">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Digite pelo menos 2 caracteres para buscar atletas da competição ativa.
                </p>
              </div>
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Desvincular atleta da academia</AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            >
              {removeAthleteMutation.isPending ? 'Desvinculando...' : 'Desvincular'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
      ...
      <AcademyFormDrawer
        ...
        isSubmitting={updateAcademyMutation.isPending}
        submitLabel="Salvar alterações"
        title="Editar academia"
        description="Atualize os dados da academia da competição ativa."
        onClose={() => setIsEditDrawerOpen(false)}
        onSubmit={handleUpdateAcademy}
      />
    </div>
  );
}
