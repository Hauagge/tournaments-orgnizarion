  ...
  useEffect(() => {
    setAreaNames((current) => {
      ...
      const next = Array.from(
        { length: areaCount },
        (_, index) => current[index] ?? `Area ${index + 1}`,
      );
      return next;
    });
  }, [areaCount]);
        ...
        <>
          <div
            className={`grid gap-6 ${
              canCreateAreas ? 'xl:grid-cols-[0.9fr_1.1fr]' : 'xl:grid-cols-1'
            }`}
          >
            <RoleGuard allow={['ORGANIZATION']}>
            ...
            <Card className="border-4 border-slate-900 p-0 shadow-[6px_6px_0_0_rgba(15,23,42,0.95)]">
              <CardContent className="space-y-4 p-5">
                ...
                {areasQuery.isLoading && (
                  <InlineState message="Carregando areas..." />
                )}
                {areasQuery.isError && (
                  <InlineState
                    message={
                ...
                {!areasQuery.isLoading &&
                  ...
                  areas.length === 0 && (
                    <InlineState
                      message="Nenhuma area cadastrada para esta competicao."
                      tone="empty"
                    />
                  )}
